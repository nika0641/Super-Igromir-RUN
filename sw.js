...
function CreateCacheFromFileList(cacheName, fileList, bypassCache)
{
	...
	.then(responses =>
	{
		...
		for (let response of responses)
		{
			...
			{
				allOk = false;
				console.error("[SW] Error fetching '" + originalUrl + "' (" + response.status + " " + response.statusText + ")");
			}
		}
		...
		.catch(err =>
		{
			...
			console.error("[SW] Error writing cache entries: ", err);
	...
	.then(data =>
	{
		...
		.then(cacheExists =>
		{
			...
			.then(mainPageUrl =>
			{
				...
				console.log("[SW] Caching " + fileList.length + " files for offline use");
